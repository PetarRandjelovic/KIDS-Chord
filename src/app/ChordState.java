...
public class ChordState {
    ...
    public static int chordHash(String value) {
        try {
            ...
            BigInteger trueHash = hash.mod(BigInteger.valueOf(CHORD_SIZE));

            //	AppConfig.timestampedStandardPrint(value + " and hashed is  " + trueHash.intValue());
            return trueHash.intValue();
    ...
    public CopyOnWriteArrayList<File> privateFiles = new CopyOnWriteArrayList<>();
    public CopyOnWriteArrayList<File> publicFiles = new CopyOnWriteArrayList<>();
    ...
    private Map<Integer, File> valueMapFilePublic = new HashMap<>();
    private Map<Integer, File> valueMapFilePrivate = new HashMap<>();

    private Map<Integer, CopyOnWriteArrayList<File>> valueMapFilePublicCopy = new ConcurrentHashMap<>();
    private Map<Integer, CopyOnWriteArrayList<File>> valueMapFilePrivateCopy = new ConcurrentHashMap<>();
    ...
    public ChordState() {
        ...
        valueMapFilePublic = new HashMap<>();
    }
    ...
    public Map<Integer, CopyOnWriteArrayList<File>> getValueMapFilePrivateCopy() {
        return valueMapFilePrivateCopy;
    }

    public Map<Integer, CopyOnWriteArrayList<File>> getValueMapFilePublicCopy() {
        return valueMapFilePublicCopy;
    }
    ...
    public void setValueMapFilePublic(Map<Integer, File> valueMapFilePublic) {
        this.valueMapFilePublic = valueMapFilePublic;
    }
    ...
    public void removeNodes(int susPort) {
        // Remove the node with the given listener port from allNodeInfo
        allNodeInfo.removeIf(n -> n.getListenerPort() == susPort);

        // Log the updated list after removal
        //    AppConfig.timestampedErrorPrint(allNodeInfo + "  REMOVENODES");
        // Clear successor table
        clearSuccessorTable();
        // Reconstruct and sort allNodeInfo
        ...
        // Update the successor table with the new list
        updateSuccessorTable();
    }
    ...
    public void putValueFile(int key, File file, String type) {
        ...
        if (isKeyMine(key)) {
            ...
            //  AppConfig.timestampedErrorPrint("NE UBACUJEM JA SAM KLJUC ZA ADD FILE "+key+" "+AppConfig.myServentInfo.getChordId()+" I "+isKeyMine(key));
            ServentInfo nextNode = getNextNodeForKey(key);
            PutFileMessage pm = new PutFileMessage(AppConfig.myServentInfo.getListenerPort(), nextNode.getListenerPort(), key, file, type);
            MessageUtil.sendMessage(pm);
        }
    }
